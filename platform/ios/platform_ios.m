...
static void report_error(mezia_platform_t *p, mezon_status_t status,
                         const char *where) {
  if (p && p->config.on_error) {
    p->config.on_error(status, where, p->config.user_data);
  }
}

void mezia_platform_report_error(mezia_platform_t *platform,
                                 mezon_status_t status, const char *where) {
  report_error(platform, status, where);
}
...
static void fill_video_config(mezia_platform_t *p) {
  ...
  p->video_config.on_nal = mezia_ios_decoder_on_nal;
...
mezia_platform_t *mezia_platform_create(const mezia_platform_config_t *config) {
  ...
  p->audio_send_queue =
      dispatch_queue_create("mezia.ios.audio.send", DISPATCH_QUEUE_SERIAL);
  p->audio_playout_queue =
      dispatch_queue_create("mezia.ios.audio.playout", DISPATCH_QUEUE_SERIAL);
  p->camera_queue =
      dispatch_queue_create("mezia.ios.camera", DISPATCH_QUEUE_SERIAL);
  p->encode_queue =
      dispatch_queue_create("mezia.ios.encode", DISPATCH_QUEUE_SERIAL);
  p->decode_queue =
      dispatch_queue_create("mezia.ios.decode", DISPATCH_QUEUE_SERIAL);
  fill_audio_config(p);
  fill_video_config(p);
...
mezon_status_t mezia_platform_start(mezia_platform_t *platform) {
  ...
  if (platform->config.enable_audio) {
    status = mezia_ios_audio_start(platform);
    if (status != MEZON_OK) {
  ...
  if (platform->config.enable_video) {
    status = mezia_ios_decoder_start(platform);
    if (status != MEZON_OK) {
      mezia_ios_audio_stop(platform);
      mezia_stop(platform->media);
      return status;
    }
    status = mezia_ios_encoder_start(platform);
    if (status != MEZON_OK) {
      mezia_ios_decoder_stop(platform);
      mezia_ios_audio_stop(platform);
      mezia_stop(platform->media);
      return status;
    }
    status = mezia_ios_camera_start(platform);
    if (status != MEZON_OK) {
      mezia_ios_encoder_stop(platform);
      mezia_ios_decoder_stop(platform);
      mezia_ios_audio_stop(platform);
      mezia_stop(platform->media);
      return status;
...
mezon_status_t mezia_platform_set_video_bitrate(mezia_platform_t *platform,
  ...
  return mezia_ios_encoder_set_bitrate(platform, bitrate_bps);
}
...
mezon_status_t mezia_platform_set_camera(mezia_platform_t *platform,
  ...
  return mezia_ios_camera_set_facing(platform, facing);
}
